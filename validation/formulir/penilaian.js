...
module.exports = function validateInput(method, path, data) {
  ...
  if (method === 'POST') {
    ...
    if (totalError) {
      ...
      if (
        !isEmpty(data.penampilan) &&
        !isEmpty(data.gerak_dasar) &&
        !isEmpty(data.keserasian) &&
        !isEmpty(data.kematangan)
      ) {
        ...
        if (Math.abs(actualTotal - expectedTotal) > 0.01) {
          errors.total = `total harus sama dengan rata-rata dari 4 kriteria (${expectedTotal.toFixed(
            2
          )}).`;
        }
      }
