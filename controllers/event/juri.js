...
Controller.create = async (req, res) => {
  try {
    ...
    const kelas = req.body.kelas || null;
    const { id_event, id_profile, penilaian } = req.body;

    /* SQL Insert Data */
    const result = await helper.runSQL({
      sql: 'INSERT INTO `tbl_juri` (`id_event`, `id_profile`, `kelas`, `penilaian`, `created_by`) VALUES (?, ?, ?, ?, ?)',
      param: [id_event, id_profile, kelas, penilaian, created_by],
    });
