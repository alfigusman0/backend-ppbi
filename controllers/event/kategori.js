...
Controller.create = async (req, res) => {
  try {
    ...
    const { id_event, ids_kelas, nama_kategori, ukuran_min, ukuran_maks, uang, status } = req.body;
...
Controller.read = async (req, res) => {
  try {
    ...
    // Tambahkan kondisi untuk range ukuran
    const addRangeCondition = (minField, maxField, minValue, maxValue) => {
      if (!isEmpty(minValue) && !isEmpty(maxValue)) {
        const condition = `? >= ${minField} AND ? <= ${maxField}`;
        const whereClause = sqlRead.includes('WHERE') ? ` AND ${condition}` : ` WHERE ${condition}`;
        sqlRead += whereClause;
        sqlReadTotalData += sqlReadTotalData.includes('WHERE')
          ? ` AND ${condition}`
          : ` WHERE ${condition}`;
        params.push(minValue, maxValue);
        totalParams.push(minValue, maxValue);
      } else if (!isEmpty(minValue)) {
        const condition = `? >= ${minField}`;
        const whereClause = sqlRead.includes('WHERE') ? ` AND ${condition}` : ` WHERE ${condition}`;
        sqlRead += whereClause;
        sqlReadTotalData += sqlReadTotalData.includes('WHERE')
          ? ` AND ${condition}`
        ...
        totalParams.push(minValue);
      } else if (!isEmpty(maxValue)) {
        const condition = `? <= ${maxField}`;
        const whereClause = sqlRead.includes('WHERE') ? ` AND ${condition}` : ` WHERE ${condition}`;
        sqlRead += whereClause;
        sqlReadTotalData += sqlReadTotalData.includes('WHERE')
          ? ` AND ${condition}`
    ...
    addRangeCondition('ukuran_min', 'ukuran_maks', ukuran_range_min, ukuran_range_maks);
...
Controller.single = async (req, res) => {
  try {
    ...
    const addRangeCondition = (minField, maxField, minValue, maxValue) => {
      if (!isEmpty(minValue) && !isEmpty(maxValue)) {
        const condition = `? >= ${minField} AND ? <= ${maxField}`;
        const whereClause = sqlSingle.includes('WHERE')
          ? ` AND ${condition}`
          : ` WHERE ${condition}`;
        sqlSingle += whereClause;
        params.push(minValue, maxValue);
      } else if (!isEmpty(minValue)) {
        const condition = `? >= ${minField}`;
        const whereClause = sqlSingle.includes('WHERE')
    ...
    // Tambahkan kondisi range ukuran
    addRangeCondition('ukuran_min', 'ukuran_maks', ukuran_range_min, ukuran_range_maks);
