...
const redisPrefix = process.env.REDIS_PREFIX + "event:settings:";
...
const checkAccess = async (req, action) => {
    const sql = {
        ...
        param: [req.authIdsLevel, 20, `%${action}%`]
    };
...
Controller.create = async (req, res) => {
    try {
        ...
        const result = await helper.runSQL({
            ...
            param: [id_event, nama_setting, setting, req.authIdUser]
        });
...
Controller.read = async (req, res) => {
    try {
        ...
        addCondition('id_setting', id_setting, 'IN');
...
Controller.single = async (req, res) => {
    try {
        ...
        addCondition('id_setting', id_setting);
