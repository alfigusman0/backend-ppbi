...
const redisPrefix = process.env.REDIS_PREFIX + "event:juara:";
...
Controller.create = async (req, res) => {
    try {
        ...
        const result = await helper.runSQL({
            sql: "INSERT INTO `tbl_juara` (`id_event`, `nama_juara`, `status`, `created_by`) VALUES (?, ?, ?, ?)",
            param: [id_event, nama_juara, status, req.authIdUser]
        });
...
Controller.read = async (req, res) => {
    try {
        ...
        // Build SQL query
        let sqlRead = "SELECT * FROM `view_juara`";
        let sqlReadTotalData = "SELECT COUNT(id_juara) as total FROM `view_juara`";
        const params = [];
        const totalParams = [];
...
Controller.update = async (req, res) => {
    try {
        ...
        await helper.runSQL({
            sql: `UPDATE tbl_juara SET ${updates.join(', ')} WHERE id_juara = ?`,
            param: [...params, id],
        });
...
Controller.delete = async (req, res) => {
    try {
        ...
        // SQL Delete Data
        await helper.runSQL({
            sql: 'DELETE FROM `tbl_juara` WHERE id_juara = ?',
            param: [id],
        });
...
Controller.single = async (req, res) => {
    try {
        ...
        // Build SQL query
        let sqlSingle = "SELECT * FROM `view_juara`";
        const params = [];
