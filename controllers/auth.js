...
Controller.callback = async (req, res) => {
  try {
    ...
    if (!data.email || !data.name) {
      return response.sc400('Failed to retrieve user information from Google.', {}, res);
    }
    ...
    if (process.env.SESSIONS === 'DATABASE') {
      ...
      await redis
        .set(key, JSON.stringify(saveRedis), 'EX', 60 * 60 * 24 * process.env.JWT_EXPIRED_IN)
        .catch(err => {
  ...
  try {
    ...
    if (!getUser[0].password) {
      return response.sc400(
        'Anda login menggunakan google. silahkan login kembali menggunakan google.',
        {},
        res
      );
    }
